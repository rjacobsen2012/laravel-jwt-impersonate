    ...
    protected function registerRoutesMacro()
    {
        ...
        $router->macro('impersonate', function () use ($router, $middleware) {
            $router->get('/impersonate/take/{id}',
                '\Incorp\Impersonate\Controllers\ImpersonateController@take')->name('impersonate')->middleware(config($middleware));
            $router->get('/impersonate/leave',
                '\Incorp\Impersonate\Controllers\ImpersonateController@leave')->name('impersonate.leave')->middleware(config($middleware));
            $router->get('/impersonate/info',
                '\Incorp\Impersonate\Controllers\ImpersonateController@info')->name('impersonate.info')->middleware(config($middleware));
        });
    }
